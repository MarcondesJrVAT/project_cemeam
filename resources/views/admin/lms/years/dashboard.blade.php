                                    ...
                                    <div class="flex">
                                        <div class="w-fit py-2 px-2 w-1/2 text-start">
                                            <div class="relative max-w">
                                                <a href="{{ route('admin.lms.years.create') }}" type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-500 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" wire:click="createUser">
                                                    <x-icons.plus-circle aria-hidden="true" />
                                                    <span class="ms-2">Novo Ano Letivo</span>
                                                </a>
                                            </div>
                                        </div>
                                        <div class="w-fit py-2 px-2 w-1/2 text-start">
                                            <div class="relative max-w-xs sr-only">
                                                ...
                                                <input type="text" wire:model="search" placeholder="Pesquisar Ano Letivo..." class="py-2 px-3 ps-9 block w-full dark:bg-gray-800 border-gray-200 dark:border-gray-500 shadow-sm rounded-lg dark:text-gray-200 focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" placeholder="Search for items">
                                    ...
                                    <div class="overflow-hidden">
                                        <table class="min-w-full divide-y divide-gray-400 dark:divide-gray-600">
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-200 uppercase">Ano</th>
                                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-200 uppercase">Atual</th>
                                            ...
                                            @foreach($years as $year)
                                                <tr class="dark:hover:bg-gray-600 hover:bg-gray-300">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-normal text-sm font-medium text-gray-800 dark:text-gray-200">{{ $year->id }}</td>
                                                    <td class="px-6 py-4 whitespace-normal text-sm font-medium text-gray-800 dark:text-gray-200">{{ $year->year }}</td>
                                                    ...
                                                    <td class="flex justify-end px-6 py-4 space-x-1 whitespace-normal text-sm font-medium">
                                                        <a href="{{ route('admin.lms.years.show', $year->id) }}" type="button" class="text-blue-500" title="Detalhes de {{ $year->year }}">
                                                            <x-icons.eye aria-hidden="true" />
                                                        </a>
                                                        <a href="{{ route('admin.lms.years.edit', $year->id) }}" type="button" class="text-blue-500" title="Editar {{ $year->year }}">
                                                            <x-icons.pencil-square aria-hidden="true" />
                                                        </a>
                                                        <form action="{{ route('admin.lms.years.destroy', $year->id) }}" method="post" onsubmit="return confirm('Tem certeza que deseja remover este ano? Esta ação é irreversível!')">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="text-blue-500" title="Remover {{ $year->year }}">
...
@push('scripts')
    <script>
        function showAlert(message) {
            ...
            alert.classList.remove('hidden'); // mostra o alerta

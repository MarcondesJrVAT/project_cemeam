<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Formulário de Edição de Ano Letivo') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-screen-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="rounded-lg bg-white dark:bg-gray-800 p-8 shadow-lg lg:col-span-3 lg:p-12">
                    <form action="{{ route('admin.lms.years.update', $year->id) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 gap-4 md:text-center sm:grid-cols-2">
                            <div>
                                <label class="dark:text-gray-200" for="year">Anos</label>
                                <select
                                ...
                                >
                                    ...
                                    @foreach($availableYears as $availableYear)
                                        <option value="{{ $availableYear }}" {{ $availableYear == $year->year ? 'selected' : '' }}>{{ $availableYear }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    required
                                >
                                    <option value="1" {{ $year->current_year === 1 ? 'selected' : '' }}>Ativo</option>
                                    <option value="0" {{ $year->current_year === 0 ? 'selected' : '' }}>Inativo</option>
                                </select>
                            </div>
                        </div>

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Formulário de Conteúdo para Aula:') }} {{ $lesson->title }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-screen-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="rounded-lg bg-white dark:bg-gray-800 p-8 shadow-lg lg:col-span-3 lg:p-12">
                    <form action="{{ route('admin.lms.lessons.contents.store', ['lesson' => $lesson]) }}" enctype="multipart/form-data" method="POST" class="space-y-4">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    required
                                >
                                    <option value="0" {{ old('is_main') === 0 ? 'selected' : '' }}>Não</option>
                                    <option value="1" {{ old('is_main') === 1 ? 'selected' : '' }}>Sim</option>
                                </select>
                                @error('is_main')<span class="text-red-500 mt-2">{{ $message }}</span>@enderror
                            </div>
                        </div>
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2" x-data="{ contentType: '{{ old('content_type') }}' }">
                            <div>
                        ...
                        <div class="mt-4 space-y-2">
                            ...
                            <a href="{{ route('admin.lms.lessons.contents.index', ['lesson' => $lesson->id]) }}" type="button" class="inline-block w-full text-center rounded-lg bg-red-400 px-5 py-3 font-medium text-gray-200 sm:w-auto">
                                Cancelar
                            </a>
                        </div>
                    </form>

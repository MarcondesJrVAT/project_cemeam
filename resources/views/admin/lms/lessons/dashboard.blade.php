                                    ...
                                    <div class="flex">
                                        <div class="w-fit py-2 px-2 w-1/2 text-start">
                                            <div class="relative max-w">
                                                <a href="{{ route('admin.lms.lessons.create') }}" type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-500 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" wire:click="createUser">
                                                    <x-icons.plus-circle aria-hidden="true" />
                                                    <span class="ms-2">Nova Aula</span>
                                                </a>
                                            </div>
                                        </div>
                                        <div class="w-fit py-2 px-2 w-1/2 text-start">
                                    ...
                                    <div class="overflow-hidden">
                                        <table class="min-w-full divide-y divide-gray-400 dark:divide-gray-600">
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-200 uppercase">Capa</th>
                                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-200 uppercase">Código</th>
                                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-200 uppercase">Título</th>
                                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-200 uppercase">Ministrantes</th>
                                            ...
                                            @foreach($lessons as $lesson)
                                                <tr class="dark:hover:bg-gray-600 hover:bg-gray-300">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-normal text-sm font-medium text-gray-800 dark:text-gray-200">{{ $lesson->image }}</td>
                                                    <td class="px-6 py-4 whitespace-normal text-sm font-medium text-gray-800 dark:text-gray-200">{{ $lesson->code }}</td>
                                                    <td class="px-6 py-4 whitespace-normal text-sm text-gray-800 dark:text-gray-200">{{ $lesson->title }}</td>
                                                    <td class="w-96 px-6 py-4 whitespace-normal text-sm text-gray-800 dark:text-gray-200">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-normal text-sm text-gray-800 dark:text-gray-200">{{ $lesson->status }}</td>
                                                    <td class="flex justify-end px-6 py-4 space-x-1 whitespace-normal text-sm font-medium">
                                                        <a href="{{ route('admin.lms.lessons.show', $lesson->id) }}" type="button" class="text-blue-500" title="Detalhes de {{ $lesson->title }}">
                                                            <x-icons.eye aria-hidden="true" />
                                                        </a>
                                                        <a href="{{ route('admin.lms.lessons.edit', $lesson->id) }}" type="button" class="text-blue-500" title="Editar {{ $lesson->title }}">
                                                        ...
                                                        <form action="{{ route('admin.lms.lessons.destroy', $lesson->id) }}" method="post" onsubmit="return confirm('Tem certeza que deseja remover esta aula? Esta ação é irreversível!')">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="text-blue-500" title="Remover {{ $lesson->title }}">

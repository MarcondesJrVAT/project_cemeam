<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Formulário de Edição do Departamento:') }} {{ $department->name }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-screen-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="rounded-lg bg-white dark:bg-gray-800 p-8 shadow-lg lg:col-span-3 lg:p-12">
                    <form action="{{ route('admin.departments.update', $department->id) }}" enctype="multipart/form-data" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ $department->responsible_id === $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                            <div>
                                ...
                                <select
                                    ...
                                    required
                                >
                                    <option>Selecione um departamento pai</option>
                                    <option value="" {{ $department->parent_id === null ? 'selected' : '' }}> ---- NENHUM ---- </option>
                                    @foreach($departments as $parent)
                                        <option value="{{ $parent->id }}" {{ $department->parent_id === $parent->id ? 'selected' : '' }}>{{ $parent->name }}</option>
                                    @endforeach
                                </select>
